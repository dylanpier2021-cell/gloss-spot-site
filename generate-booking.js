...
.slot-alt-head{margin-top:18px;color:var(--text);font-size:1rem;line-height:1.5;padding:18px;background:var(--soft);border-radius:6px;border-left:3px solid var(--primary)}
.slot-alt-head strong{color:var(--ink)}
.slot-alt-grid{display:grid;grid-template-columns:repeat(3,1fr);gap:10px;margin-top:14px}
.slot-alt{padding:14px 12px;border:1.5px solid var(--line);border-radius:6px;background:#fff;cursor:pointer;text-align:left;transition:all .2s;font-family:'Inter Tight',sans-serif}
.slot-alt:hover{border-color:var(--primary);background:#fff;transform:translateY(-1px);box-shadow:0 8px 18px -10px rgba(2,136,138,.3)}
.slot-alt-day{font-family:'Bricolage Grotesque',sans-serif;font-weight:700;color:var(--ink);font-size:.98rem;display:block}
.slot-alt-meta{font-size:.78rem;color:var(--muted);margin-top:4px;display:block;letter-spacing:.04em}
...
async function loadSlots(dateStr) {
  ...
  try {
    ...
    const allDays = Object.keys(data)
      ...
      .filter((k) => Array.isArray(data[k]?.slots) && data[k].slots.length > 0)
      .sort();

    let pickedSlots = (data[dateStr] && data[dateStr].slots) || [];
    if (S.duration >= 4 && pickedSlots.length > 0) pickedSlots = [pickedSlots[0]];

    if (pickedSlots.length > 0) {
      // Picked day has slots — render them as the main grid.
      let html = '<div class="slot-grid">';
      pickedSlots.forEach((iso) => {
    ...
    // No slots on the picked day — show alternate days from the 14-day window.
    if (allDays.length === 0) {
      block.innerHTML = '<div class="slot-empty">No times available in the next 14 days. Please <a href="' + ${JSON.stringify(PHONE_TEL)} + '">call ${PHONE}</a> to book by phone.</div>';
      return;
    }
    const next = allDays.slice(0, 6);
    let html = '<div class="slot-alt-head"><strong>No openings on ' + fmtDateLong(dateStr) + '.</strong> Closest available days below — click one to load times.</div>';
    html += '<div class="slot-alt-grid">';
    next.forEach((day) => {
      const count = (data[day].slots || []).length;
      html += '<button type="button" class="slot-alt" data-day="' + day + '"><span class="slot-alt-day">' + fmtDateLong(day) + '</span><span class="slot-alt-meta">' + count + ' time' + (count === 1 ? '' : 's') + ' open</span></button>';
    });
    html += '</div>';
    block.innerHTML = html;
    block.querySelectorAll(".slot-alt").forEach((b) => {
      b.addEventListener("click", () => {
